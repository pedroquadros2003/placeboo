...
class LocalBackend:
    ...
    def __init__(self, base_path: str):
        ...
        self.accounts_path = os.path.join(self.base_path, 'account.json')
    ...
    def process_outbox(self):
        ...
        """
        outbox_messages = self._read_json(self.outbox_path)
        if not outbox_messages:
            return

        inbox_messages = self._read_json(self.inbox_path)
        new_inbox_messages = []
        ...
        for msg in outbox_messages:
            ...
            elif obj == "linking_accounts" and action == "respond_to_invitation":
                if payload.get("response") == "accept":
    ...
    def _generate_unique_id(self, id_type):
        ...
        ids_path = os.path.join(self.base_path, filename)
        existing_ids = self._read_json(ids_path, [])

        while True:
            new_id = str(random.randint(10000000, 99999999))
            if new_id not in existing_ids:
                existing_ids.append(new_id)
                self._write_json(ids_path, existing_ids)
                return new_id
